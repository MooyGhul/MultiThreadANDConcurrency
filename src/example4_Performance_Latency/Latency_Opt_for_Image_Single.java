package example4_Performance_Latency;
...
public class Latency_Opt_for_Image_Single {
    ...
    public static void recolorImage(BufferedImage originalImage, BufferedImage resultImage, int leftCorner, int topCorner, int width, int height){
        for(int x = leftCorner; x<leftCorner+width && x<originalImage.getWidth(); x++){
            for(int y = topCorner; x<topCorner+height && y<originalImage.getHeight(); y++){
    ...
    public static void recolorPixel(BufferedImage originalImage, BufferedImage resultImage, int x, int y ){
        int rbg = originalImage.getRGB(x,y);

        int red = getRed(rbg);
        int green = getGreen(rbg);
        int blue = getBlue(rbg);
